...
<head>
    ...
    <style>
        * {
        ...
        .dot.input { background: #667eea; }
        .dot.output { background: #764ba2; }
        .dot.cache-create { background: #f093fb; }
        .dot.cache-read { background: #4facfe; }
    ...
    <script>
        ...
        function updateDashboard(data) {
            ...
            uniqueHosts.forEach(hostname => {
                const hostProjects = status.entries
                    .filter(e => e.hostname === hostname)
                    .flatMap(e => e.projects);
                const totalTokens = hostProjects.reduce((sum, p) => 
                    sum + p.tokens.inputTokens + p.tokens.outputTokens + 
                    p.tokens.cacheCreationTokens + p.tokens.cacheReadTokens, 0
                );
            ...
            Object.entries(projectTokens)
                .sort((a, b) => {
                    const totalA = a[1].tokens.inputTokens + a[1].tokens.outputTokens + 
                                   a[1].tokens.cacheCreationTokens + a[1].tokens.cacheReadTokens;
                    const totalB = b[1].tokens.inputTokens + b[1].tokens.outputTokens + 
                                   b[1].tokens.cacheCreationTokens + b[1].tokens.cacheReadTokens;
